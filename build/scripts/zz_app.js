...
function addLoading() {
  return makeDomElement('div', { className: 'loading flex flex-column'},
...
function renderChartComponent(_indicatorId, _chart) {
  ...
  var chartComponent = makeDomElement('div', { id: _chart.id, className: 'chart'},
    ['div', { className: 'head' },
      ['h3', _chart.title],
      ['div', { className: 'break-line' }, ['br']],
      ['p', { className: 'paragraph', innerHTML: _chart.description }],
      ['div', { className: 'break-line' }, ['br'], ['br']]
    ],
    ['div', { className: 'referenceContainer' },
      ['div', { className: 'break-line' }, ['br']],
      ['span', { className: 'references' }],
      ['div', { className: 'break-line' }, ['br']],
      ['div', { className: 'break-line' }, ['hr']]
    ],
...
function downloadFilesToChart(_chart) {
  ...
  promises.catch(() => {
    var indicatorsError = [], indicators = arguments['0'].indicators, chart = arguments['0'].id;
  ...
  promises.then(() => { injectChartData(arguments['0']); renderChart(arguments['0']); });
}
// Completa la tarjeta del grñafico con la metadata del indicador.
// (Update: 25.08.2017)
function injectChartData(_chart) {
  _chart.indicators.forEach((_indicator) => {
    var component = makeDomElement('p', {},
      ['div', { className: 'reference-round-line', style: { backgroundColor: _indicator.color } }],
      ['span', { innerHTML: _indicator.short_name }]
    );
  ...
  function makeDomElement(desc) {
    ...
    if (typeof attributes === 'object' && attributes !== null && !isArray(attributes)) {
      ...
      for (var attr in attributes) {
        switch (attr) {
          case 'style': for (var style in attributes[attr]) { el.style[style] = attributes[attr][style]; } break;
          default: el[attr] = attributes[attr]; break;
        }  
      }
    ...
    for (var i = start; i < desc.length; i++) {
      if (isArray(desc[i])) { el.appendChild(makeDomElement(desc[i])); }
...
function changeView(_containerId, _cardId) {
  ...
  if (container === '#chartsContainer') { STORAGE.activeCard = _cardId; node.show(); }
...
function renderCardComponent(_card) {
  var cardComponent = makeDomElement('div', { id: _card.id, className: 'card'},
    ['h3', { innerHTML: _card.title }],
    ['div', { className: 'break-line' }, ['br'], ['br'], ['hr'], ['br'], ['br']],
    ['h4', { innerHTML: _card.short_name }],
    ['div', { className: 'break-line' }, ['br']],
    ['p', { className:'frequency' }],
    ['div', { className: 'break-line' }, ['br'], ['br']],
    ['p', { className:'units_representation' }],
    ['div', { className: 'break-line' }, ['br']],
    ['p', { className:'units' }],
    ['div', { className: 'break-line' }, ['br']],
    ['div', { className: 'mini-chart' }],
    ['div', { className: 'break-line' }, ['br']],
    ['p', { className:'human_frecuency' }],
    ['div', { className: 'break-line' }, ['br'], ['br']],
    ['button', { className: 'button', onclick: () => { changeView('chartsContainer', _card.id); requestAllCharts(_card.id); } }, ['span', { className: 'button-waves', innerHTML: 'Ver más gráficos' }]],
    ['div', { className: 'break-line' }, ['br']],
  ...
  var url_ext = `http://meconcd.mecon.gob.ar/public.php?service=files&t=e9cd25ad56afd6c53514d9c9d191f494&download&path=//${ _card.id }.json`,
