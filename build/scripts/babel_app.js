...
function requestAllCharts(_indicatorId) {
    ...
    cardData.charts.forEach((_chart) => {
...
function renderChart(_chart) {
    ...
    chartLine = d3.line().curve(d3.curveMonotoneX).x((d) => chartScaleX(d.date)).y((d) => chartScaleY(d.value)).defined((d) => {
        // console.log(typeof d.value);
        return d.value != null;
    });
    rangeLine = d3.line().curve(d3.curveMonotoneX).x((d) => rangeScaleX(d.date)).y((d) => rangeScaleY(d.value)).defined((d) => {
        // console.log(typeof d.value);
        return d.value != null;
    });
...
function getDataset(_id) {
    return STORAGE[_id].dataset.filter((_item) => _item.identifier === _id)[0];
}

function getDistributions(_distributions, _data) {
    var distributions = [];

    _distributions.map((_distribution) => {
        var result = null;

        _data.forEach((_dataDist) => {
    ...
    distributions.forEach((_dist, k) => {
        ...
        elementDom.push(
            [
                ...
                [
                    'button',
                    {className: 'button', download: true},
                    [
                        'span',
...
function addMetadata() {
    ...
    data.forEach((_dataset) => {
        ...
        }, _dataset.dataset_identifier).then(() => renderDataset(_dataset));
    });
}
