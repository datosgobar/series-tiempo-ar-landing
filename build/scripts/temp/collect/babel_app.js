...
function renderChartComponent(_indicatorId, _chart) {
    ...
    // ],
    ['div', { className: 'break-line' }, ['div', { className: 'br' }]]], ['div', { className: 'chart-svg' }], ['div', { className: 'break-line' }, ['div', { className: 'br' }]], ['div', { className: 'modal-share' }, ['input', { id: 'share-' + _chart.id, type: 'checkbox', className: 'share-open' }], ['label', { htmlFor: 'share-' + _chart.id, className: 'share-open-button hamburger-dark' }, ['span', { className: 'hamburger-1' }], ['span', { className: 'hamburger-2' }], ['span', { className: 'hamburger-3' }]], ['button', { className: 'share-item button buttonCircle', title: 'Embeber', onclick: function onclick(e) {
...
function tooltipsCollapse(_chart) {
    ...
    indicatorsDom.forEach(function (_indicator) {
    ...
    elements_asc.forEach(function (v, k) {
        if (k !== elements_asc.length - 1) {
            var _start = elements_asc[k],
                startPosition = updateTranslatePositionY(_start)[1],
    ...
    elements_desc.forEach(function (v, k) {
        ...
        if (k !== elements_desc.length - 1) {
            ...
            var _start2 = elements_desc[k],
                startAdd = _start2.querySelector('.boxText'),
                end = elements_desc[k + 1],
                endAdd = end.querySelector('.boxText'),
                startPosition = updateTranslatePositionY(_start2)[1] + updateTranslatePositionY(startAdd)[1],
...
function start(_cardId, _indicatorId) {
    ...
    if (_cardId === undefined || _indicatorId === undefined) {
        ...
        download.then(function () {
            ...
            })[0].charts.filter(function (_chart) {
